...
export default function FeedPage() {
  ...
  return (
    <div className="min-h-screen relative font-sans text-slate-800" style={{ backgroundColor: GRAY_BG }}>
      ...
      {/* Location Modal with Radar Animation - RESTORED FROM hello.tsx */}
      {isLocationModalOpen && (
      ...
      {/* Share Modal - RESTORED FULL VERSION */}
      {shareModalOpen && (
      ...
      <div className="max-w-screen-2xl mx-auto w-full grid grid-cols-1 lg:grid-cols-[260px_1fr_300px] gap-8 px-4 lg:px-8 py-8">
        ...
        {/* Left Sidebar - Navigation & Shortcuts - RESTORED FULL VERSION */}
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map((item, index) => (
                <motion.div
                  ...
                  className={`relative group overflow-hidden rounded-3xl bg-white shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer border border-gray-100 ${getCardStyle(index)}`}
                >
                  {/* Media Content */}
                  <div className="absolute inset-0 bg-gray-100">
                    {item.type === "video" ? (
                      ...
                      >
                        <p className="text-white font-bold text-2xl leading-tight">{item.text}</p>
                      </div>
                    ) : (
                      <img src={item.src!} alt={item.text} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
                    )}
                  </div>
                  ...
                  {item.type !== 'text' && (
                    <div className="absolute bottom-0 left-0 p-6 w-full transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                      <h3 className="text-white font-bold text-xl leading-tight shadow-black drop-shadow-md line-clamp-2">{item.text}</h3>
        ...
        {/* Right Sidebar - Trends & Suggestions - RESTORED FULL VERSION */}
        <div className="hidden lg:block space-y-6 sticky top-24 h-fit">
